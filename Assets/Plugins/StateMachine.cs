...
namespace IceMilkTea.Core
{
    ...
    public class ImtStateMachine<TContext, TEvent>
    {
        ...
        private readonly HashSet<Func<Type, State>> stateFactorySet;
        ...
        /// <exception cref="InvalidOperationException">ステートマシンは、既に起動中です</exception>
        /// <exception cref="InvalidOperationException">ステートクラスのインスタンスの生成に失敗しました</exception>
        public void AddTransition<TPrevState, TNextState>(TEvent eventId) where TPrevState : State, new()
        ...
        ///     現在のステートマシンは、別のスレッドによって更新処理を実行しています。[UpdaterThread={LastUpdateThreadId},
        ...
        private void DoHandleException(Exception exception)
        {
            ...
            if (UnhandledExceptionMode == ImtStateMachineUnhandledExceptionMode.CatchException &&
            ...
            if (UnhandledExceptionMode == ImtStateMachineUnhandledExceptionMode.CatchStateException &&
                currentState != null)
        ...
        private TState GetOrCreateState<TState>() where TState : State, new()
        {
            ...
            var newState = CreateStateInstanceCore<TState>() ??
                           throw new InvalidOperationException("ステートクラスのインスタンスの生成に失敗しました");
